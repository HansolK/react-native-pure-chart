...
export default class ColumnChart extends Component {
  constructor(props) {
    super(props);
    let defaultGap =
      this.props.defaultColumnWidth + this.props.defaultColumnMargin;
    ...
    if (seriesCount > 0) {
      ...
      for (let i = 0; i < dataCount; i++) {
        renderColumns.push(
          <ColumnChartItem
            key={i}
            seriesArray={this.state.sortedData}
            dataIndex={i}
          ...
          styles.chartView,
          {
            ...
            marginBottom:
              ...
              this.state.guideArray.length > 0
                ? -1 * this.state.guideArray[0][2] * this.props.minValue
                : null
          }
        ]}
      >
    ...
    if (typeof selectedIndex === "number" && selectedIndex >= 0) {
      ...
      for (let i = 0; i < this.state.sortedData.length; i++) {
        ...
        if (series.data[selectedIndex]["x"]) {
          tooltipRenders.push(
            <Text key={"tooltipTitle-" + i} style={styles.tooltipTitle}>
              Time: {series.data[selectedIndex]["x"]}
            </Text>
          );
        }
        tooltipRenders.push(
          ...
          >
            ...
            <Text style={styles.tooltipValue}>
              {numberWithCommas(series.data[selectedIndex]["y"], false)}
            </Text>
          </View>
        );
      }
...
const styles = StyleSheet.create({
  ...
  tooltipTitle: { fontSize: scale(10) },
  tooltipValue: { fontWeight: "bold", fontSize: scale(15) },
  tooltipColor: {
    width: scale(10),
    height: scale(5),
    marginRight: scale(3),
